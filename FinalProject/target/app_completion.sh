...
# =======================
#
# Bash completion support for the `app` command,
...
#
# 2. Open a new bash console, and type `app [TAB][TAB]`
#
...
# 'app (..)'. By reading entered command line parameters,
...
# _complete_app finds which commands and subcommands have been specified
# on the command line and delegates to the appropriate function
# to generate possible options and subcommands for the last specified subcommand.
function _complete_app() {
...
# Generates completions for the options and subcommands of the `app` command.
function _picocli_app() {
  # Get completion data
  local curr_word=${COMP_WORDS[COMP_CWORD]}
...
# Generates completions for the options and subcommands of the `hello` subcommand.
function _picocli_app_hello() {
  # Get completion data
  local curr_word=${COMP_WORDS[COMP_CWORD]}
...
function _picocli_app_add() {
  ...
  local commands=""
  local flag_opts=""
  local arg_opts="-n --name -t --title -y --type -c --credit -d --departmentId -r --term -l --location -f --finalTime -i --time"
...
# `app`, `app.sh`, and `app.bash` commands.
# Uses the bash `complete` builtin (see [6]) to specify that shell function
# `_complete_app` is responsible for generating possible completions for the
# current word on the command line.
# The `-o default` option means that if the function generated no matches, the
# default Bash completions and the Readline default filename completions are performed.
complete -F _complete_app -o default app app.sh app.bash
